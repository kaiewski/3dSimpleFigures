...
class Canvas(FigureCanvas):
	...
	def draw_capacitor(self, point_quanity: int, XYZ_start: list, XYZ_end: list, electrode_distance: float) -> None:
		...
		[x, y, z] = self.calculate_capacitor_sizes(point_quanity, XYZ_start, XYZ_end, electrode_distance)

		N = point_quanity

		self.axes.fill_between(x[0], y[0], z[0], x[1], y[1], z[1], alpha=0.5, edgecolor='k')
		self.axes.fill_between(x[2], y[2], z[2], x[3], y[3], z[3], alpha=0.5, edgecolor='k')
		self.setParent(self.parent_app)

	def calculate_capacitor_sizes(self, point_quanity: int, XYZ_start: list, XYZ_end: list, electrode_distance: float) -> list[list, list, list]:
	...
	def draw_arrow_axes(self, arrow_length: float=0.1, enable_axes: bool=True) -> None:
		...
		if enable_axes:
			...
			self.axes.quiver(*origin, *x_axis, color=colors[0], arrow_length_ratio=0.1, linewidth=1)
			self.axes.quiver(*origin, *y_axis, color=colors[1], arrow_length_ratio=0.1, linewidth=1)
			self.axes.quiver(*origin, *z_axis, color=colors[2], arrow_length_ratio=0.1, linewidth=1)
			
			self.axes.text(arrow_length * 1.1, 0, 0, "X", color=colors[0], fontsize=5)
			self.axes.text(0, arrow_length * 1.1, 0, "Y", color=colors[1], fontsize=5)
			self.axes.text(0, 0, arrow_length * 1.1, "Z", color=colors[2], fontsize=5)

	def clear_plot(self) -> None:
		"""Clear an axes and plot."""
		self.axes.cla()
		self.axes.set_axis_off()
		self.axes.set(xlim=(0, 1), ylim=(0, 1), zlim=(0, 1))
